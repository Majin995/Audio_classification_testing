"""Compile Phase H results into a markdown summary.

Walks lightning_logs/phaseH_*/version_0/, pulls best val/macro_precision from
...
def collect() -> list[dict]:
    rows = []
    for run_dir in sorted(LOG_ROOT.glob("phaseH_*")):
        v0 = run_dir / "version_0"
        if not v0.exists():
            continue
        metrics_csv = v0 / "metrics.csv"
        log_path    = LOG_ROOT / "phaseH_console" / f"{run_dir.name}.log"
...
def write_report(rows: list[dict], out_path: Path) -> None:
    ...
    lines = [
        ...
        f"test/μP={base['test_mp']:.4f}  MP@0.85=0.7518",
        "",
        "Sorted by **test/μP** (the production-relevant metric).",
        "",
        "| Run | val/μP | Δval | test/μP | Δtest | "
        "c2 Tanker | c3 Tug | MP@0.85 |",
        "|---|---|---|---|---|---|---|---|",
    ]
...
if __name__ == "__main__":
    rows = collect()
    out = LOG_ROOT / "phaseH_summary.md"
