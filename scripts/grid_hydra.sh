#!/usr/bin/env bash
# Grid search for HydroHydra — precision-first sweep.
#
# Axes (training-time):
...
#
# Fixed: use_gabor=1, use_scattering=1 (full dual-stream).
# Post-hoc: temperature + per-class thresholds fit automatically.
#
# Total: 4 × 2 × 2 × 2 = 32 runs.
#
# Usage:
#   export DATA_DIR=/abs/path/to/Split1s
#   bash scripts/grid_hydra.sh
#   DRY_RUN=1 bash scripts/grid_hydra.sh
...
m = HydroHydra.load_from_checkpoint(best_ckpt, map_location=device, strict=False).to(device).eval()
  ...
  for G in "${GAMMAS[@]}"; do
    for S in "${SMOOTHINGS[@]}"; do
      for GW in "${GAMBLER_WEIGHTS[@]}"; do
        i=$((i + 1))
        RUN="grid_hydra_m${M}_g${G}_s${S}_gw${GW}"

        CMD="$PYTHON training/train_hydra.py \
            --run_name \"grid_hydra/$RUN\" \
            --data_dir \"$DATA_DIR\" \
        ...
        BEST_EPOCH=$(echo "$BEST_CKPT" | grep -oE "hydra-[0-9]+" | grep -oE "[0-9]+")
