#!/usr/bin/env bash
# Phase I LPC retry — re-run γ.1 LPC with the tanh-squashed Levinson fix.
#
# I2's original run produced NaN loss from epoch 0 because the unstable
# Levinson recursion let reflection coefficients exceed |k|=1 on coherent
# ship-machinery harmonics, blowing up E and propagating NaN. Fix shipped
# 2026-05-06: ``k = tanh(-acc / E)`` enforces |k| < 1.
...
run() {
  ...
  echo "════════════════════════════════════════════════════════════════"
  echo " PHASE I LPC-FIX — starting run: $name"
  echo " log: $logf"
  ...
  local rc=$?
  echo "════════════════════════════════════════════════════════════════"
  echo " PHASE I LPC-FIX — finished:   $name  (exit=$rc)"
  echo " finished: $(date -Is)"
  echo "════════════════════════════════════════════════════════════════"
  return 0
}

run phaseI_I2b_lpc_fix "${R1[@]}" --use_lpc

echo "════════════════════════════════════════════════════════════════"
echo " PHASE I LPC-FIX — completed."
echo "════════════════════════════════════════════════════════════════"
