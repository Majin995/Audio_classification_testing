"""
Training script for HydroALSI (ALSI-Hybrid).

Usage:
    export DATA_DIR=/path/to/Split1s
    python training/train_alsi.py [options]
"""
...
def get_args():
    p = argparse.ArgumentParser(description="Train HydroALSI")

    # Data
    p.add_argument("--data_dir",       default=os.environ.get("DATA_DIR", ""))
    p.add_argument("--batch_size",     type=int,   default=32,
                   help="Use smaller batch due to Wav2Vec2 memory footprint")
    ...
    p.add_argument("--freeze_wav2vec", type=lambda x: x.lower() == "true",
                   default=True, help="Freeze Wav2Vec2 weights (true/false)")
    p.add_argument("--cqt_bins",       type=int,   default=84)
    ...
    p.add_argument("--run_name",       default="hydro_alsi")
...
def main(args=None):
    ...
    print(f"\nHydroALSI — {n_params / 1e6:.2f}M trainable parameters "
          f"(Wav2Vec2 {'frozen' if args.freeze_wav2vec else 'unfrozen'})")

    callbacks = [
        ModelCheckpoint(
            monitor="val/f1", mode="max", save_top_k=3,
            filename="alsi-{epoch:03d}-f1{val/f1:.4f}", verbose=True,
        ),
