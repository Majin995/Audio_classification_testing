"""
Training script for HydroDCN (Deep Complex Network).

Usage:
    export DATA_DIR=/path/to/Split1s
    python training/train_dcn.py [options]
"""
...
def get_args():
    p = argparse.ArgumentParser(description="Train HydroDCN")
    ...
    # Model
    p.add_argument("--dcmf_templates", type=int,   default=16)
    p.add_argument("--complex_depth",  type=int,   default=3)
    ...
    p.add_argument("--run_name",       default="hydro_dcn")
...
def main(args=None):
    ...
    model = HydroDCN(
        ...
        dcmf_templates=args.dcmf_templates,
        complex_depth=args.complex_depth,
    ...
    print(f"\nHydroDCN — {n_params / 1e6:.2f}M trainable parameters")

    callbacks = [
        ModelCheckpoint(
            monitor="val/f1", mode="max", save_top_k=3,
            filename="dcn-{epoch:03d}-f1{val/f1:.4f}", verbose=True,
        ),
