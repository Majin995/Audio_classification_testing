"""
Training script for HydroWave1D — spectrogram-free, high-precision UATR.

Monitors ``val/macro_precision`` for checkpointing.  After training, runs
post-hoc temperature scaling + per-class threshold search (imported from
``training.train_precise``).

Usage:
    export DATA_DIR=/path/to/Split1s
    python training/train_wave1d.py [options]
"""
...
def get_args():
    p = argparse.ArgumentParser(description="Train HydroWave1D (spectrogram-free)")
    ...
    p.add_argument("--run_name",        default="hydro_wave1d")
...
def main(args=None):
    ...
    model = HydroWave1D(
        ...
        d_model=args.d_model, stem_stride=args.stem_stride,
    ...
    print(f"\nHydroWave1D — {n_params / 1e6:.2f}M trainable parameters")
    ...
    ckpt_cb = ModelCheckpoint(
        ...
        filename="wave1d-{epoch:03d}-p{val/macro_precision:.4f}",
    ...
    if not args.skip_calibration and best_path:
        ...
        cal_model = HydroWave1D.load_from_checkpoint(
