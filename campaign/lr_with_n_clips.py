"""LR stacker that adds log(n_clips) per source as a structural feature.

Rationale: per-source clip count is class-correlated (Cargo median 229,
Tanker median 120, Tug median 390). It's a data-distribution feature that
the base ckpts cannot see, and may help disambiguate the
unanimous-but-wrong Cargo→Tanker errors.

Selection: subset×C on val 5-fold OOF macro-F1. Test touched once.
"""
...
def src_features(P_stack, G, include_nclips, eps=1e-8):
    M, _, C = P_stack.shape
    n_extra = 1 if include_nclips else 0
    X = np.zeros((len(G), M * C + n_extra), dtype=np.float32)
    y = np.zeros(len(G), dtype=np.int64)
    for gi, (idx, gy, _) in enumerate(G):
        ch = P_stack[:, idx]
        X[gi, :M * C] = np.log(ch.mean(1) + eps).flatten()
        if include_nclips:
            X[gi, M * C] = np.log1p(len(idx))
        y[gi] = gy
    return X, y
...
def run_oof_lr_bag(X, y, C, n_bags=10, k_folds=5):
    ...
    for bag in range(n_bags):
        ...
        for tr, te in skf.split(np.zeros_like(y), y):
            m = LogisticRegression(C=C, max_iter=5000, random_state=bag).fit(X[tr], y[tr])
            P_oof[te] += m.predict_proba(X[te])
            b = LogisticRegression(C=0.1, max_iter=5000, class_weight='balanced',
                                   random_state=bag).fit(X[tr], (y[tr] == TUG).astype(int))
...
def main():
    ...
    print('\n=== Phase 1: subset × C × include_nclips search on val 5-fold OOF (LR x10 bag) ===')
    t0 = time.time()
    best = (-1.0, None, None, None, None)  # f1, subset_name, subset, C, include_nclips
    for name, subset in cands:
        V = np.stack([pool[s][0] for s in subset])
        for include_nclips in (False, True):
            Xv, ysv = src_features(V, GV, include_nclips)
            for C in C_grid:
                P_oof, p_tug_oof = run_oof_lr_bag(Xv, ysv, C)
                pred = P_oof.argmax(1)
                pred = np.where(p_tug_oof > 0.95, TUG, pred)
                m = metrics(ysv, pred)
                tag = 'nclips' if include_nclips else 'noNC'
                print(f'  [{time.time()-t0:5.1f}s] {name:24s} C={C:6.3f} {tag:6s} '
                      f'F1={m["f1"]:.4f} mP={m["macroP"]:.4f} R={m["recall"]:.4f}')
                if m['f1'] > best[0]:
                    best = (m['f1'], name, subset, C, include_nclips)

    f1_best, name_best, sub_best, C_best, nclips_best = best
    print(f'\nWINNER subset={name_best} C={C_best} nclips={nclips_best}  val_OOF F1={f1_best:.4f}')

    # Phase 2: refit, evaluate test once
    ...
    Xv, ysv = src_features(V_best, GV, nclips_best)
    Xt, yst = src_features(T_best, GT, nclips_best)

    n_bags = 10
    P_test = np.zeros((len(yst), 4), dtype=np.float32)
    p_tug_test = np.zeros(len(yst), dtype=np.float32)
    for bag in range(n_bags):
        m = LogisticRegression(C=C_best, max_iter=5000, random_state=bag).fit(Xv, ysv)
    ...
    out_dir = Path('lightning_logs/lr_with_n_clips')
    out_dir.mkdir(parents=True, exist_ok=True)
    joblib.dump({
        'method': 'LR bagged (n_bags=10) + Tug LR booster + log(n_clips)',
        'ckpt_subset': sub_best, 'subset_name': name_best,
        'C_best': C_best, 'include_nclips': nclips_best,
        'val_oof_f1': float(f1_best),
        'test_f1': float(m['f1']), 'test_macroP': float(m['macroP']),
    }, out_dir / 'lr_nclips.joblib')
    print(f'\nSaved → {out_dir}/lr_nclips.joblib')
