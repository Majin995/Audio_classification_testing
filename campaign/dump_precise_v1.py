"""Dump probs for HydroPrecise (v1) ckpts on Classifier_Dataset.

Adapted from dump_precise.py (which targets V2). The precise-013-p0.7444 ckpt
is the v1 HydroPrecise architecture (CQT branch + Gabor + DEMON).
"""
import argparse
import json
...
def main():
    ...
    print(f"[dump-precise] data_dir={args.data_dir} num_classes={nc}", flush=True)
    ...
    for ck in args.ckpts:
        ...
        if out_path.exists():
            print(f"[dump-precise] skip (exists): {out_path}", flush=True)
            meta["ckpts"].append({"ckpt": ck, "stem": stem, "npz": str(out_path)})
            continue
        print(f"[dump-precise] {ck}", flush=True)
        try:
            m = HydroPrecise.load_from_checkpoint(ck, map_location=device, strict=False)
        ...
        try:
            ...
            print(f"  val ok: {vp.shape}", flush=True)
        except FileNotFoundError as e:
            print(f"  no val: {e}", flush=True)
        except Exception as e:
            print(f"  val failed: {e}", flush=True)
        try:
            dm.setup()
            tp, ty = collect(m, dm.test_dataloader(), device, nc)
            out_dict["test_probs"], out_dict["test_y"] = tp, ty
            print(f"  test ok: {tp.shape}", flush=True)
        except FileNotFoundError as e:
            print(f"  no test: {e}", flush=True)
        except Exception as e:
            print(f"  test failed: {e}", flush=True)
        if out_dict:
            np.savez(out_path, **out_dict)
            sizes = "  ".join(f"{k}={v.shape}" for k, v in out_dict.items() if v.ndim > 0)
            print(f"  wrote {out_path}  {sizes}", flush=True)
            meta["ckpts"].append({"ckpt": ck, "stem": stem, "npz": str(out_path)})
        del m
